...
export default function DashboardClient() {
  ...
  async function handleRename() {
    ...
    if (error) {
      ...
      showToast("err", error.message || "Rename failed.");
      return;
    }
  ...
  async function handleConfirmDelete() {
    ...
    if (error) {
      ...
      showToast("err", error.message || "Delete failed.");
      return;
    }
  ...
  return (
    <main className="min-h-screen bg-white text-gray-900">
      ...
      {toast && (
        <div className="fixed left-1/2 top-5 z-[100] -translate-x-1/2 w-[90%] max-w-md pointer-events-none">
          <div className={cx(
            ...
            toast.type === "ok" ? "border-green-100 text-green-700" : "border-red-100 text-red-600"
          )}>
            {toast.text}
          </div>
        </div>
      ...
      {/* DELETE MODAL */}
      {deletingId && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-gray-900/20 backdrop-blur-sm px-6" onClick={() => setDeletingId(null)}>
          <div className="w-full max-w-sm rounded-3xl border border-gray-200 bg-white p-6 shadow-2xl animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-red-50 text-red-600 mb-4">
              <span className="text-2xl">⚠️</span>
            </div>
            <h3 className="text-lg font-extrabold tracking-tight text-gray-900">Delete Survey?</h3>
            <p className="mt-2 text-sm text-gray-500 leading-relaxed">
              Are you sure you want to delete <span className="font-bold text-gray-900 italic">"{deletingTitle}"</span>? 
              This will permanently remove all questions and responses.
            </p>
            <div className="mt-6 flex flex-col gap-2">
              <button onClick={handleConfirmDelete} className="w-full rounded-2xl bg-red-600 py-3 text-sm font-bold text-white hover:bg-red-700 shadow-md transition-colors">Delete Forever</button>
              <button onClick={() => setDeletingId(null)} className="w-full rounded-2xl border border-gray-200 py-3 text-sm font-bold text-gray-600 hover:bg-gray-50 transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
                      ...
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div onClick={(e) => e.stopPropagation()}>
                          <CardMenu
                            ...
                            onDelete={() => {
                              setDeletingId(s.id);
                              setDeletingTitle(s.title || "Untitled survey");
                            }}
                          />
                        </div>
                      </div>
